...
- Last updated: `2026-05-23 12:37:52`
...
- [12:59] [代码] A股 Radar 主线分类准度 Ledger 已实现：新增 `a_share_radar_classification_ledger.py`，从最新 A股短线计划登记每个主题的发现日、阶段、总分、广度代理、成员/强势/涨停代理、D5/D10/D14 到期复核项，并输出 `backtest_results/a_share_radar_classification_ledger/latest_ledger.{json,csv}` 与桌面 `报表输出/LATEST/A股Radar主线分类准度Ledger_LATEST.{md,json,csv}`。已接入 `a_share_radar_daily_loop.py`、晚间指导邮件和 `morning_brief.py`；daily email 会显示主线分类到期复核数量。当前已有 4 条样本，均为 2026-05-17 计划主题，D5 到期复核 4 条：人形机器人/半导体设备/氟化工/AI应用。验证：`py_compile` 通过；`a_share_radar_daily_loop.py --dry-run --skip-review --skip-backfill` 通过且未拉富途行情。
- [13:28] [代码] 轻量人工主题证据工作流已实现：新增 `manual_theme_evidence_ledger.py`，创建并读取两个固定 INBOX：`/Users/zhangkun/Desktop/AI个人投资公司/信息源扫描/Theme_Evidence_Inbox/A股Radar_人工信息搜集_INBOX.md` 与 `.../V6AB_人工信息搜集_INBOX.md`。用户负责发现和初判政策/产业/公告/财报/SEC/13F/盘面线索，系统负责结构化记录、生成 `Theme_Evidence_人工搜集_LATEST.{md,json,csv}`、D5/D10/D14 复核提醒。已接入 `morning_brief.collect_workflow_actions()`，因此 Investment System Board / Daily Light Check 会显示 Theme Evidence 今日搜集任务；A股 daily loop 也新增 `manual_theme_evidence_ledger` 步骤。新增说明文档：`系统优化升级依据/人工主题证据搜集工作流_v1.md`。验证：`py_compile` 通过；`a_share_radar_daily_loop.py --dry-run --skip-review --skip-backfill` 通过且未拉行情；直接检查 workflow actions 已出现 A股/V6AB 两条搜集任务。
- [13:43] [代码] 已区分 A股/V6AB 自动拉取与人工搜集边界，并实现今日第一条自动源：新增 `a_share_futu_plate_heat_scan.py`，通过 Futu OpenD `get_plate_list/get_plate_stock/get_market_snapshot` 扫描 A股板块热度，计算上涨比例、强势股、涨停代理、成交额、领涨样本；不拉历史 K 线，不消耗 K线额度，OpenD 不可用时输出 `SKIPPED/TIMEOUT` 且不阻塞 daily loop。已接入 `a_share_radar_daily_loop.py`，输出 `报表输出/LATEST/A股富途板块热度自动扫描_LATEST.{md,json,csv}`。今天运行结果：Futu OpenD `127.0.0.1:11111` 不可达，报告正常 `SKIPPED`；daily loop dry-run 通过。边界文档写入 `系统优化升级依据/A股_V6AB_自动拉取与人工搜集边界_20260523.md`：同花顺/东方财富/财联社保持人工；巨潮/交易所公告作为下一步低频元数据自动检索；V6AB 自动源继续使用价格/SEC/13F/事件日历，产业链报道保持人工。
- [14:39] [修正] Futu OpenD 不可达原因已查明：用户端 Futu OpenD 实际已监听 `127.0.0.1:11111`，默认沙箱访问 localhost socket 会报 `PermissionError: Operation not permitted`，需用更高权限运行 Futu OpenD 相关脚本。提权后 `a_share_futu_plate_heat_scan.py` 成功扫描 8 个板块；因今天是周六，`change_rate` 全为 0，只能证明链路可用，真实热度需交易日盘中/收盘后重跑。脚本已修复：节流到默认 8 个板块、3.2s 间隔，过滤非沪深普通股，避免频率限制和北交所未知代码错误。另修复 Theme Evidence INBOX：两个文件所有权改回 `zhangkun:staff`，内容改为自由粘贴区，不再要求用户填复杂表格；`manual_theme_evidence_ledger.py` 已支持自由文本块解析为 `raw_manual_paste` 待整理记录。
