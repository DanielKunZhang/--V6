...
def reconcile_state(state: dict[str, Any], order_rows: dict[str, dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for item in pending:
        ...
        if dealt_qty > 0:
            if side == "BUY":
                positions[ticker] = positions.get(ticker, 0.0) + dealt_qty
            elif side == "SELL":
                positions[ticker] = max(positions.get(ticker, 0.0) - dealt_qty, 0.0)
...
def write_report(path: Path, summary: dict[str, Any], state_path: Path) -> None:
    lines = [
        ...
        "## Events",
        "",
        "| order_id | ticker | side | status | dealt_qty | avg_price |",
        "| --- | --- | --- | --- | ---: | ---: |",
    ]
