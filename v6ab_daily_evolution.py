...
def render_daily_report(
    ...
    override_evidence_counts = (
        override_evidence.get("decision_counts", {})
    ...
    parent_child_tilt_decision = (
        parent_child_tilt_backtest.get("decision", {})
    ...
    legacy_fn_decision = (
        legacy_false_negative_audit.get("decision", {})
    ...
    override_rows = override_evidence.get("rows", []) if isinstance(override_evidence.get("rows"), list) else []
    lines += [
        "",
        "## Override Evidence 审查",
        "",
        "| asof | child | parent | decision | market adv | actionable | tickers | score adv |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: |",
    ]
    for row in sorted(
        override_rows,
        key=lambda item: (
            item.get("review_decision") == "OVERRIDE_EVIDENCE_CANDIDATE",
            item.get("review_decision") == "WATCH_OVERRIDE_EVIDENCE",
            float(item.get("quality_score_advantage", 0.0) or 0.0),
        ),
        reverse=True,
    )[:8]:
        child = row.get("child", {}) if isinstance(row.get("child"), dict) else {}
        lines.append(
            f"| `{row.get('asof')}` | `{row.get('child_theme')}` | `{row.get('parent_theme')}` | "
            f"`{row.get('review_decision')}` | {float(row.get('market_advantage', 0.0) or 0.0):+.1f} | "
    ...
    tilt_rows = parent_child_tilt_backtest.get("rows", []) if isinstance(parent_child_tilt_backtest.get("rows"), list) else []
    lines += [
        "",
        "## Parent + Child Tilt Backtest",
        "",
        "| candidate | ann | maxDD | Sharpe | 2024-2026 ann | 2021 ann | 2020 ann |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in tilt_rows:
        if row.get("candidate") not in {
            "baseline_v2_v6ab_dynamic_b",
            "original_pit_guarded_v6ab_dynamic_b",
            "parent_child_tilt_guarded_v6ab_dynamic_b",
            "overlap_guarded_tilt_v6ab_dynamic_b",
        }:
            continue
        stats = row.get("stats", {})
        periods = row.get("periods", {})
    ...
    month_review = (
        parent_child_tilt_backtest.get("month_review", [])
        if isinstance(parent_child_tilt_backtest.get("month_review"), list)
        else []
    )
    if month_review:
        lines += [
    ...
    fn_rows = (
        legacy_false_negative_audit.get("false_negative_rows", [])
        if isinstance(legacy_false_negative_audit.get("false_negative_rows"), list)
        else []
    )
    if fn_rows:
        lines += [
            "",
            "### Legacy Preservation 漏保审计",
            "",
            "| date | skipped | skipped delta vs PIT | decision |",
            "| --- | --- | ---: | --- |",
        ]
        for row in fn_rows[:8]:
            lines.append(
