...
- [2026-05-13] [代码] 已新增 `v6a_core_deterministic_replay.py` 与 `v6_strategy_lab/configs/v6a_core_replay_bridge_v1.json`，专门为 `V6-A core baseline / balanced challenger` 生成 runner-compatible replay artifacts；每个 profile 都会额外输出单独的 `daily / summary / composite` 文件，避免被 legacy `ATTACK_EQUAL_REPLAY` 多候选平均逻辑污染。
- [2026-05-13] [发现] `V6-A core replay bridge` 继续支持 challenger：`balanced challenger` 约 `Full Ann +33.97% / MaxDD -22.34% / Sharpe 1.11 / OOS Ann +44.18% / OOS Sharpe 1.30 / annual turnover 5.25x`；baseline 约 `+26.18% / -26.06% / 0.88 / +36.78% / 1.13 / 9.01x`。更重要的是：`balanced challenger rolling_3y_worst_ann = +0.78%`，baseline 为 `-4.73%`。
- [2026-05-13] [决策] 这一步明确了两个边界：(1) 近端 production-path 优先级上，`balanced challenger > current V6-B`；(2) `ATTACK_EQUAL_REPLAY` live pilot 与 `V6-A core engine` 升级线不是同一条策略，必须分轨治理。当前 live pilot 不变；下一步是等待 fresh replay row 后，把 `balanced challenger` 单候选工件接入 guarded-runner dry-run / preview。
- [2026-05-13] [文件] 已新增 `v6_strategy_lab/reports/2026-05-13_v6a_balanced_challenger_cutover_sop_v1.md`，把 `2026-05-26` 之后的 cutover 流程固定为标准作业：先收集 evidence pack，再输出 `GO_CUTOVER / HOLD_OLD_BASELINE / PAUSE_V6`，只有在旧 live pilot 与新 challenger preview 都通过时才允许进入受控迁移。
